(() => {
    ...
    const trottle = (fn, wait) => {
        let shootBeCalled = true;
        return (...args) => {
            if (!shootBeCalled) return;
            shootBeCalled = false;
            setTimeout((() => {
                shootBeCalled = true;
            }), wait);
    ...
    const loging = value => console.log(value);
    const debounceLoging = trottle(loging, 1e3);
    document.getElementById("debounce")?.addEventListener("input", (e => {
        debounceLoging(e.target.value);
    }));
    document.addEventListener("DOMContentLoaded", (() => {
        ...
        const handleSubmit = async event => {
            ...
            try {
                ...
                const [name, email, phone] = [ formData.get("name"), formData.get("email"), formData.get("phone") ];
                if (!name || !email || !phone) throw new Error("Все поля обязательны для заполнения");
        ...
        const startButton = document.getElementById("getCamera");
